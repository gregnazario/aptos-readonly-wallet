...
- **Transaction simulation** (`simulate` config, default on) previews what an intercepted transaction *would* do. It runs **only from the extension pages** (popup log + approval window), never the MAIN-world wallet (page CSP would block the fullnode fetch; extension pages have `<all_urls>` host permission). `src/shared/simulate.ts` is split into pure, unit-tested pieces (`buildSimulationInput` reconstructs a single-signer entry-function call from the logged `pretty` JSON — reviving `"123n"` bigints — and `summarizeResponse` normalizes the fullnode `UserTransactionResponse`) plus a thin injectable network layer (`simulatePayload`/`Simulator`, so tests never hit the network). It simulates **without a key** via the SDK's `simulate.simple` with no `signerPublicKey` (skips the auth-key check). `src/shared/simulation-view.ts` renders the shared panel (badge + gas + Retry button); multi-agent and raw/script txns are reported as `unsupported`. The popup auto-runs only the newest tx (others get a button) to avoid a burst of fullnode calls; the approval window auto-runs the one request being decided.
